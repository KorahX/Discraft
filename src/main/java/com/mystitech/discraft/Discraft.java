...
/**
 * Created by KorahX on 22/03/2023.
 */

@Plugin(id = "discraft",
        name = "Discraft",
        description = "A utility Discord to Minecraft chat bridge plugin",
        authors = {"KorahX"},
        dependencies = {
...
public class Discraft {
    ...
    public void onStartingServer(GameStartingServerEvent event) {
        ...
        this.init().thenRun(() -> {
            ...
            CommandSpec cs = CommandSpec.builder()
                    ...
                    .permission("discraft.admin.broadcast")
                    .arguments(
            ...
            logger.error("Error loading Discraft: ", throwable.getCause());
            return null;
        });

        Sponge.getServiceManager().provide(PermissionService.class).ifPresent(svc -> {
            svc.getDefaults().getTransientSubjectData().setPermission(Sets.newHashSet(), "discraft.chat", Tristate.TRUE);
            svc.getDefaults().getTransientSubjectData().setPermission(Sets.newHashSet(), "discraft.mention", Tristate.TRUE);
        });
    }
    ...
    public void reload(GameReloadEvent event) {
        this.stop().thenCompose(v -> this.init())
                ...
                .exceptionally(throwable -> {
                    logger.error("Error reloading Discraft: ", throwable.getCause());
                    return null;
                });
    }
    ...
    CompletableFuture<Void> init() {
        return CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                if (e.getMessage().contains("Was shutdown trying to await status")) {
                    logger.error("JDA couldn't start and didn't throw any errors. Make sure your bot " +
                            "has the SERVER MEMBERS INTENT enabled in the application page!");
                } else {
            ...
            if (throwException) {
                RuntimeException exception = new RuntimeException("Discraft errored and could not start, check the logs for the error!");
    ...
    CompletableFuture<Void> stop() {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            logger.error("Error stopping Discraft: ", throwable.getCause());
            return null;
        });
    }
    ...
    private void registerListeners() {
        ...
        if (config.CHANNELS.USE_NUCLEUS) {
            ...
            if (Sponge.getPluginManager().getPlugin("nucleus").isPresent()) {
                ...
                logger.error(" ");
                logger.error(" Discraft is configured to hook into Nucleus, but it isn't loaded! Please disable using-nucleus or load Nucleus on your server!");
                logger.error(" ");
            }
            ...
            if (Sponge.getPluginManager().getPlugin("ultimatechat").isPresent()) {
                ...
                logger.error(" ");
                logger.error(" Discraft is configured to hook into UltimateChat, but it isn't loaded! Please disable using-ultimatechat or load UltimateChat on your server!");
                logger.error(" ");
                useVanillaChat = true;
            }
        } else {
